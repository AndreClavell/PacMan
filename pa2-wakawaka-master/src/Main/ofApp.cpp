...
//--------------------------------------------------------------
void ofApp::setup(){
	music.load("sounds/Elevator Music.mp3");
	ofSetFrameRate(30);
...
void ofApp::update(){
	if (currentState != nullptr){
		...
		if(currentState->hasFinished()){
			...
			{
				CurrentHS=gameState->getMap()->getPlayer()->getHScore();
				goState= new GameOverState(gameState->getMap());
				goState->setName(player);
				currentState= goState;
			}else if(currentState->getNextState() == "Game"){
				gameState = new GameState();
				gameState->getMap()->getPlayer()->setPrevHScore(CurrentHS);
